...
define('FILE_OTHER','yiyou/');
class Control_install_yy {
    public static function Action_index($r){
    	...
    	if($handle = opendir(DIR_HDT_INSTALL.'yiyou')){
    	...
    	Flight::display("install_yy/index.html", $result);
    }
    ...
    public static function Action_install_product($r){
        $file_excel = DIR_HDT_INSTALL . FILE_OTHER .iconv("UTF-8", "GBK", FILE_HDT_PRODUCT_DETAIL);
                ...
                if($product_id){
                    ...
                    foreach($color_list as $color_string){
                        ...
                        if($color_string){
                            ...
                            $ProductColor->create_color($product_id, $color_id, $skc_id, '');
                        }
                    }
                    if(!$IMAGE_KUANHAO){
    ...
    public static function Action_install_product_group($r){
        $file_excel = DIR_HDT_INSTALL .FILE_OTHER. iconv("UTF-8", "GBK", FILE_HDT_PRODUCT_GROUP);
        $file_ini   = DIR_HDT_INSTALL . FILE_HDT_IMPORT_INI;
        if(file_exists($file_ini)){
            $ini    = json_decode(file_get_contents($file_ini), true);
        }
        $IMAGE_EXTENDS  = $ini['IMAGE_EXTEND']  ? $ini['IMAGE_EXTEND']  : 'jpg';
        $GROUP_KUANHAO  = 'kuanhao';// ? $ini['GROUP_KUANHAO'] : 'kuanhao';
        $result     = array('error'=>0);
        $Product        = new Product;
        ...
        if(file_exists($file_excel)){
            ...
            while($row = $Excel->nextRow()){
                ...
                $group_name         = $row[$rid++];
                if(!$group_name) $group_name = "搭配" . $group_dp_num;
                ...
                if($group_id){
                    $product_kuanhao = $row[$rid++];
                    $product_color = $row[$rid++];
                    $product_id = $Product_cache_get($product_kuanhao);
                    $color_id = $Keywords_cache_get($product_color);     
    ...
    public static function Action_install_product_display($r){
        $file_excel = DIR_HDT_INSTALL . FILE_OTHER.iconv("UTF-8", "GBK", FILE_HDT_PRODUCT_DISPLAY);
        $file_ini   = DIR_HDT_INSTALL . FILE_HDT_IMPORT_INI;
        if(file_exists($file_ini)){
            $ini    = json_decode(file_get_contents($file_ini), true);
        }
        $IMAGE_EXTENDS      = $ini['IMAGE_EXTEND']  ? $ini['IMAGE_EXTEND']  : 'jpg';
        $DISPLAY_KUANHAO    = 'kuanhao';//   ? $ini['DISPLAY_KUANHAO']   : 'bianhao';
        $result     = array('error'=>0);
        $Product        = new Product;
        $ProductColor   = new ProductColor;
        $Keywords       = new Keywords;
