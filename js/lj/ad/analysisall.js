...
define(['jquery', 'app/pager'], function(require, exports, module) {
   ...
   '#a6c96a'
    ...
    $('#HDT-sub-exp').on('click', function(e){
    	var area1	=	$('#HDT-select-menu').find('[name=area1]').val();
    	var area2	=	$('#HDT-select-menu').find('[name=area2]').val();
    	var fliter_uid	=	$('#HDT-select-menu').find('[name=fliter_uid]').val();
    	fliter	=	"<input type='hidden' name='area1' value='"+area1+"'></input><input type='hidden' name='area2' value='"+area2+"'></input><input type='hidden' name='fliter_uid' value='"+fliter_uid+"'></input>";
    	require.async(['jquery/jquery.ui'], function(ui){
            dialog = $('<div>').html($('#tmpl-exp').html()).dialog({width:240,height:140, autoOpen:true});
            $('.fliter').html(fliter);
        });
    });
});