...
define(['jquery', 'app/pager'], function(require, exports, module) {
   ...
   '#a6c96a'
    ...
    var api = new pager('/analysis/list', {t:t}, {autorun:false, aftercallback:function(html){
        if(currentChart && html && $('#HDT-main tr[data-u]').length){
            currentFunc.call();
        }
    }});

    var api1 = new pager('/analysis/list', {t:t,status_val:2}, {autorun:false, id:'#HDT-main1', aftercallback:function(html){
    ...
    $('#HDT-select-menu').on('change', 'select', function(e){
        ...
        if(api1){
        	if(target.name == "category_id") {
                ...
                api1.set(target.name, target.value, true);
                api1.set('classes_id', 0, true);
                api1.reload();
            }else{
            	api1.set(target.name, target.value);
            }
        }
        if(target.name == "category_id") {
            $.get('/location/get_classes_list', {category_id:target.value}, function(html){
