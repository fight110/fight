...
define(['jquery', 'app/pager', 'app/lazy'], function(require, exports, module) {
    ...
    function check_error () {
        ...
        $(".HDT-order-num").each(function(){
        	var that	=	this;
        	var product_id	=	$(that).attr("product_id"),moq = $(that).attr("data-moq");
        	var p_num	=	that.innerHTML >> 0;
        	/*$('.HDT-order-num').find('[product_id='+product_id+']').each(function(){
        		p_num	+= this.innerHTML >> 0;
        	})*/
        	if(p_num < moq)
        		that.style.border = config.border;
        });
    }
	...
	$('.orderViewBody').on('touchstart mousemove',".touch_image",function(){
		var that = this;
