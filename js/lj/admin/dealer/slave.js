...
define(['jquery', 'app/selects'], function(require, exports, module) {
    var $search  = $('#HDT-search-slave'), $searchlist = $("#HDT-search-list"), Selects = require('app/selects'),
        $slavelist = $('#HDT-slave-list'), user_id = $search[0].getAttribute('data-user-id');
    ...
    $search.on('change', function(e){
        ...
        $.post('/dealer/slave_user_list', query, function(html){
    ...
    $('body').on('click', ":checkbox", function(e){
        ...
        if(user_id && user_slave_id){
            $.post('/dealer/set_slave/', {user_id:user_id, user_slave_id:user_slave_id, status:checked ? 1 : 0});
        }
    	...
    	if(checked==true){
    	    for(var i=0; i<len; i++)
    	    {
    	        if((el[i].nodeName=="SPAN"))
    	        {
    	        	...
    	        	user_list.push({user_id:user_id, user_slave_id:user_slave_id});
    	        }
    	    }
    	}else{
    		for(var i=0; i<len; i++)
    ...
    $('body').on('click','#HDT-sub-btn',function(e){//保存按钮
    	if(confirm("确认保存？")){
    		...
    		if(user_list.length){
    			$.post('/dealer/set_slave_list/', {user_list:user_list});
    		}
    	}
	});
	$searchlist.on('click', ':checkbox', function(e){
        var target  = e.currentTarget, parent = $(target).parent();
        /*parent.appendTo($slavelist);*/
        user_list.push({user_id:user_id, user_slave_id:target.value});
    });
});