...
SpriteMe.marginX = 10;
SpriteMe.marginY = 10;
SpriteMe.marginWiggle = 2000; // how much we're willing to extend the margin (in addition to marginX and marginY) if necessary to accomodate more images
SpriteMe.resizeWiggle = 2000; // how much we're willing to let repeating bg images be resized so they can be sprited with other bg images of different sizes
...
SpriteMe.getBgImages = function() {
	...
	for ( var i = 0; i < nElems; i++ ) { 
		...
		var imageUrls = SpriteMe.getStyleAndUrl(elem, 'backgroundImage', true);
		for (var imageUrlsI in imageUrls) {
			var imageUrl = imageUrls[imageUrlsI];
			...
			if ( imageUrl && !SpriteMe.skipImage(imageUrl) ) {
				...
				new SpriteMe.ImageElement(elem, bgImage, imageUrlsI);  // this adds the element to bgImage
			}
		}
	}
	...
	if ( bSecondPass ) {
		...
		for ( var i = 0; i < aStylesheets.length; i++ ) {
			...
			for ( var r = 0, nRules = aRules.length; r < nRules; r++ ) {
				...
				if (rule.style.backgroundPosition === undefined && rule.style.backgroundImage === undefined)
					continue;
				
				var bgPoss = rule.style.backgroundPosition.split(', ');
				var bgImages = rule.style.backgroundImage.split(', ');
				for (var bgI in bgImages) {
					var bgPos = bgPoss[bgI];
					var bgImage = bgImages[bgI];
		...
		for ( var i = 0; i < SpriteMe.aSprites.length; i++ ) {
			...
			if ( spriteObj.spriteUrl ) {
				for ( var e = 0; e < spriteObj.bgImages[0].imgElements.length; e++ ) {
					var elemObj = spriteObj.bgImages[0].imgElements[e];
					if ( elemObj.bExported && false ) {
						continue;
					}
					
					var bgPos = SpriteMe.getStyleAndUrl(elemObj.elem, "backgroundPosition", false, elemObj.bgIndex);
					var bgImage = SpriteMe.getStyleAndUrl(elemObj.elem, "backgroundImage", false, elemObj.bgIndex);
					...
					if ( ! aRules ) {
						...
						for ( var iRule = 0; iRule < aRules.length; iRule++ ) {
							...
							if ( ! ruleObj.bExported || true ) {
								...
								var oldBgImages = oldBgImage.split(', ');
								oldBgImages[elemObj.bgIndex] = bgImage;
								ruleObj.rule.style.backgroundImage = oldBgImages.join(', ');
								
								// background-position:
								
								console.log(ruleObj.rule.selectorText);
								
								if (/:hover/.test(ruleObj.rule.selectorText)) {
									...
									if (! ruleObj.bExported) {
										...
										var oldBgPoss = oldBgPos.split(', ');
										for(var i = 0; i <= elemObj.bgIndex; ++i) {
											if (oldBgPoss[i] === undefined || oldBgPoss[i] === '')	{
												oldBgPoss[i] = '0 0';
											}
										}
										var possOld = oldBgPoss[elemObj.bgIndex].split(' ');
										var possNew = bgPos.split(' ');
										var newVal = [];

										console.log(possOld);

										// remove "px" and get int value
										for (var i = 0; i < 2; ++i) {
											possOld[i] = parseInt((new String(possOld[i])).replace('px', '').replace('0%', '0'));
											possNew[i] = parseInt((new String(possNew[i])).replace('px', '').replace('0%', '0'));

											newVal[i] = possOld[i] + possNew[i] + 'px';
										}

										oldBgPoss[elemObj.bgIndex] = newVal.join(' ');

										//oldBgPoss[elemObj.bgIndex] = bgPos;
										console.log(ruleObj.rule.style.backgroundPosition);
										ruleObj.rule.style.backgroundPosition = oldBgPoss.join(', ');
										console.log(ruleObj.rule.style.backgroundPosition);
								...
								if ( "undefined" === typeof(hChanges[ruleObj.url]) ) {
									hChanges[ruleObj.url] = {};
									hChanges[ruleObj.url]['_'] = "<div style='margin-bottom: 8px;'>Merge these changes into the CSS rules in <a href='" + 
								...
								hChanges[ruleObj.url][ruleObj.rule.selectorText] = /*"<em>This is one of multiple rules that use this background image:</em><br>" + */ruleObj.getCssHtml(oldBgImage);
								elemObj.bExported = true;
					...
					/*
						...
						if ( ! ruleObj.bExported ) {
							...
							// background-position:
								...
								console.log(ruleObj.rule.cssText);
							var oldBgPos = ruleObj.rule.style.backgroundPosition;
							var oldBgPoss = oldBgPos.split(', ');
							for(var i = 0; i <= elemObj.bgIndex; ++i) {
								if (oldBgPoss[i] === undefined || oldBgPoss[i] === '')	{
							...
							console.log(oldBgPoss[elemObj.bgIndex]);
							console.log(bgPos);
							var possOld = oldBgPoss[elemObj.bgIndex].split(' ');
							var possNew = bgPos.split(' ');
							var newVal = [];
							...
							for (var i = 0; i < 2; ++i) {
								possOld[i] = parseInt((new String(possOld[i])).replace('px', '').replace('0%', '0'));
								possNew[i] = parseInt((new String(possOld[i])).replace('px', '').replace('0%', '0'));
...
//							ruleObj.rule.style.backgroundPosition = bgPos;
				...
				for ( var iRule = 0; iRule < aRules.length; iRule++ ) {
					...
					if (typeof hChanges[ruleObj.url] !== 'string') {

						var s = "";
						for (var i in hChanges[ruleObj.url])
							s += hChanges[ruleObj.url][i];
						hChanges[ruleObj.url] = s;
...
SpriteMe.getStyleAndUrl = function(elem, prop, bGetUrl, bgIndex) {
	var val = "";
	...
	if ( "backgroundPosition" === prop && SpriteMe.bIE ) {
		var posX = SpriteMe.getStyleAndUrl(elem, 'backgroundPositionX', false, bgIndex);
		posX = ( "left" == posX ? "0%" : ( "center" == posX ? "50%" : ( "right" == posX ? "100%" : posX ) ) );
		var posY = SpriteMe.getStyleAndUrl(elem, 'backgroundPositionY', false, bgIndex);
...
SpriteMe.setStyleAndUrl = function(elem, prop, val, bSetUrl, bgIndex) {
	if ( bSetUrl ) {
		val = "url('" + val + "')";
	}
	...
	if ( "float" === prop ) {
		...
		var aPos = val.split(' ');
		SpriteMe.setStyleAndUrl(elem, 'backgroundPositionX', aPos[0], false, bgIndex);
		SpriteMe.setStyleAndUrl(elem, 'backgroundPositionY', aPos[1], false, bgIndex);
		return;
	}
	...
	if (prop === "backgroundPosition") {
		jQueryProp = 'background-position';
	} else if (prop === "backgroundImage") {
		jQueryProp = 'background-image';
	}
	
	if (bgIndex !== undefined) {
		var currentVal = $(elem).css(jQueryProp); //elem.style[prop];
		...
		// pricitani k puvodni hodnote
		/*
		if (prop === "backgroundPosition") {
			var possOld = vals[bgIndex].split(' ');
			var possNew = val.split(' ');
			console.log(elem);
			console.log(possOld);
			console.log(possNew);
			var newVal = [];
			...
			vals[bgIndex] = newVal.join(' ');
			console.log(vals[bgIndex]);
		} else if (prop === "backgroundImage") {
			vals[bgIndex] = val;
		}
		*/
...
SpriteMe.BgImage.prototype = {
	...
	getDataFromElement: function(imgElem) {
		imgElem.backgroundRepeat = SpriteMe.getStyleAndUrl(imgElem.elem, 'backgroundRepeat', false, imgElem.bgIndex);
		...
		imgElem.backgroundPosition = SpriteMe.getStyleAndUrl(imgElem.elem, "backgroundPosition", false, imgElem.bgIndex);
		/*
...
//
////////////////////////////////////////////////////////////////////////////////
SpriteMe.ImageElement = function(elem, bgImage, bgIndex) {
...
SpriteMe.ImageElement.prototype = {
	...
	spritify: function(url, bgImage) { // ImageElement
		...
		if ( aPos ) {
			this.oldBgPos = SpriteMe.getStyleAndUrl(this.elem, "backgroundPosition", false, this.bgIndex);
			SpriteMe.setStyleAndUrl(this.elem, 'backgroundPosition', aPos[0] + 'px ' + aPos[1] + 'px', false, this.bgIndex);
			this.oldBgImage = SpriteMe.getStyleAndUrl(this.elem, "backgroundImage", false, this.bgIndex);
			this.oldBgImageInline = this.elem.style.backgroundImage;
			SpriteMe.setStyleAndUrl(this.elem, 'backgroundImage', url, true, this.bgIndex);
		}
		else {
